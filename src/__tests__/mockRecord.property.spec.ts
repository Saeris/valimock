...
import { boolean, number, parse, picklist, pipe, record, string, uuid } from "valibot";
...
describe(`mockRecord property-based`, () => {
  it(`record with string keys: every mock round-trips through parse`, () => {
    ...
    fc.assert(
      fc.property(valueShapeArb, (valueSchema) => {
        const schema = record(string(), valueSchema as v.GenericSchema) as unknown as v.GenericSchema;
  ...
  it(`record with uuid keys: keys are uuids`, () => {
    const schema = record(pipe(string(), uuid()), number()) as unknown as v.GenericSchema;
  ...
  it(`record may shrink when key schema produces duplicates (picklist)`, () => {
    ...
    const schema = record(picklist([`a`, `b`]), number()) as unknown as v.GenericSchema;
